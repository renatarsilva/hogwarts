    ...
    xl:px-10 max-lg:py-4"
      >
        <a className="block w-[12rem] xl:mr-8">
          <img src={logo} width={60} height={30} alt="Brainwave" />
        </a>
        ...
        text-2xl uppercase text-n-1 transition-colors hover:text-hogwarts-gold-light ${
          item.onlyMobile ? "lg:hidden" : ""
        } px-6 py-6 md:py-8 lg:-mr-0.25 
        lg:text-xs lg:font-semibold ${
          item.url === pathname.hash ? "z-2 lg:text-n-1" : "lg:text-n-1/50"
        } lg:leading-5 lg:hover:text-hogwarts-gold-light xl:px-12`}
              >
                {item.title}
              </a>
            ))}
